...
mkdir c:\script-tmp
Invoke-WebRequest -URI https://github.com/ikashi-sunhine/pack-vnc-install/archive/refs/heads/main.zip -UseBasicParsing -outfile c:\script-tmp\sc.zip
Expand-Archive -Path c:\script-tmp\sc.zip  -DestinationPath c:\script-tmp\file -Force
C:\script-tmp\file\pack-vnc-install-main\vnc.msi
...
if($arh -eq 1) {
   Write-Output "x32"
Invoke-WebRequest -URI https://github.com/ikashi-sunhine/zabbix-agen-cl/raw/main/zabbix_agent-6.4.4-windows-i386-openssl.msi -UseBasicParsing -outfile c:\script-tmp\x32.msi
C:\script-tmp\x32.msi
}
elseif ($arh -eq 2) {
    Write-Output "x64"
Invoke-WebRequest -URI https://github.com/ikashi-sunhine/zabbix-agen-cl/raw/main/zabbix_agent-6.4.4-windows-amd64-openssl.msi -UseBasicParsing -outfile c:\script-tmp\x64.msi
